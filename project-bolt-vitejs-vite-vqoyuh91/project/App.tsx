...
// Note: runCsoAlgorithm import removed, logic moved to worker
...
// Import the worker
import ScheduleWorker from './workers/schedule.worker?worker';
...
const App: React.FC = () => {
  ...
  useEffect(() => {
    ...
    workerRef.current = new ScheduleWorker();
    ...
    workerRef.current.onmessage = (e) => {
      ...
      if (type === 'SUCCESS') {
        if (result.bestFitness > 20000) {
            setSchedule(null);
            setError([{ 
                ruleId: "OPTIMIZATION_FAILED", 
                message: `Failed to generate a usable schedule. The best solution found had too many conflicts (Fitness Score: ${result.bestFitness.toFixed(0)}). Please check constraints (staff availability, qualifications, etc.) and try again.` 
            }]);
            setFitnessScore(result.bestFitness);
            setGaStatusMessage("Schedule generation failed. Quality threshold not met.");
        } else {
            const scheduleWithIds = result.schedule ? result.schedule.map((entry: any) => ({...entry, id: entry.id || generateScheduleEntryId() })) : [];
            ...
            if (result.finalValidationErrors.length > 0) {
               ...
               // AUTO-RECORD SYSTEM BEST
               // If the fitness is very low (e.g., 0 for no hard violations), save it automatically.
               if (result.bestFitness < 50) {
                   ScheduleLearningService.submitFeedback({
                       schedule: scheduleWithIds,
                       rating: 5,
                       violationsCount: 0,
                       violationsDetail: [],
                       feedbackText: "System Auto-Recorded Optimal Schedule",
                       teamId: "SYSTEM"
                   }).catch(err => console.error("Failed to auto-save optimal schedule", err));
               }
            }
            setFitnessScore(result.bestFitness);
        ...
        setError([{ ruleId: "WORKER_ERROR", message: workerError }]);
        setGaStatusMessage(`Error: ${workerError}`);
      }
  ...
  const handleDateChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setError(null); setGaStatusMessage(null); setFitnessScore(null);
    if (dateString) {
  ...
  const handleGenerateSchedule = useCallback(async () => {
    ...
    setLoadingState({ active: true, message: 'Optimizing Schedule (Background Worker)...' });
    setError(null); setSchedule(null); setGaStatusMessage(null); setFitnessScore(null);

    try {
        // Fetch learning context and data needed for worker
        ...
        const learningContext = { topRatedSchedules, learnedLunchTimes, violationPenalties: {} };
        const currentBaseSchedules = baseScheduleService.getBaseSchedules();
        ...
        if (workerRef.current) {
            workerRef.current.postMessage({
                ...
                callouts,
                baseSchedules: currentBaseSchedules,
                learningContext
            });
        } else {
      ...
      console.error("Error initiating schedule generation:", e);
      setError([{ ruleId: "INIT_ERROR", message: e.message || "Could not start scheduler." }]);
      setLoadingState({ active: false, message: '' });
    }
  ...
  const handleOptimizeCurrentScheduleWithGA = useCallback(async () => {
    ...
    setLoadingState({ active: true, message: 'Evolving Current Schedule (Background Worker)...' });
    setError(null);
    ...
    try {
       ...
       if (workerRef.current) {
            workerRef.current.postMessage({
                ...
                selectedDate: selectedDate.toISOString(),
                callouts,
                initialSchedule: schedule, // Pass current schedule for evolution
                baseSchedules: currentBaseSchedules,
                learningContext
            });
        }
      ...
      <main className="container mx-auto px-4 sm:px-6 py-8 flex-grow">
        <div className="bg-white p-1 sm:p-2 rounded-lg shadow-xl mb-8">
          ...
          <div className="p-3 sm:p-4 md:p-6">
            {activeTab !== 'schedule' && activeTab !== 'adminSettings' && !isSessionModalOpen && (
            ...
            {activeTab === 'schedule' && !isSessionModalOpen && (
                <ValidationInfoDisplay 
                    errors={error} 
                    score={fitnessScore}
            ...
            {activeTab === 'adminSettings' && bulkOperationSummary && bulkOperationSummary.errorCount > 0 && (
                <ValidationInfoDisplay 
                    errors={bulkOperationSummary.errors.map(e => ({ruleId: `ROW_${e.rowNumber}`, message: `${e.message} ${e.rowData ? `(Data: ${e.rowData.substring(0,100)}...)` : ''}`}))} 
                    title="Bulk Operation Issues"
                />
            )}
