...
// Generate consistent pastel colors from a string (Client ID)
const getClientColorStyles = (id: string) => {
  let hash = 0;
  for (let i = 0; i < id.length; i++) {
    hash = id.charCodeAt(i) + ((hash << 5) - hash);
  }
  const h = Math.abs(hash) % 360;
  // Pastel background, darker border, dark text
  return {
    backgroundColor: `hsl(${h}, 85%, 94%)`,
    borderColor: `hsl(${h}, 60%, 75%)`,
    color: `hsl(${h}, 40%, 20%)`
  };
};
      ...
      <div className="bg-gradient-to-r from-blue-50 to-sky-50 p-4 rounded-lg border border-blue-200 shadow-sm">
        <div className="flex flex-wrap items-center gap-4 text-sm">
          ...
          <div className="ml-auto flex items-center gap-3">
            <span className="text-xs text-slate-500 italic mr-1">Colors indicate unique clients</span>
            <div className="flex items-center gap-1"><span className="w-3 h-3 bg-yellow-100 border border-yellow-300 rounded"></span><span className="text-xs text-slate-600">Lunch/Indirect</span></div>
          </div>
        </div>
      </div>
                   ...
                   {sortedTeamIds.map(teamId => (
                    teamsData[teamId].therapists.map(therapist => {
                        ...
                        if (entryForCell) {
                            ...
                            let colorClasses = styling.classes;

                            if (entryForCell.clientId) {
                                const clientColors = getClientColorStyles(entryForCell.clientId);
                                dynamicStyles = {
                                    backgroundColor: clientColors.backgroundColor,
                                    borderColor: clientColors.borderColor,
                                    color: clientColors.color
                                };
                                colorClasses = ''; // Use dynamic colors instead
                            }

                            return (
                                <td key={entryForCell.id}
                                    className={`p-1.5 border border-slate-200 text-xs relative group cursor-move hover:ring-2 hover:ring-blue-400 hover:shadow-lg transition-all ${colorClasses}`}
                                    style={dynamicStyles}
                                    rowSpan={rowSpan}
                                    draggable="true"
                                ...
                                <div className="flex items-start justify-between gap-1">
                                  ...
                                  <div className="flex flex-col gap-1">
                                    <svg className={`w-3 h-3 opacity-60 group-hover:opacity-100 transition-opacity ${entryForCell.clientId ? '' : 'text-slate-400'}`} style={entryForCell.clientId ? { color: dynamicStyles.color } : {}} fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 6h16M4 12h16M4 18h16" />
                                    </svg>
                                    <PencilIcon className={`w-3 h-3 opacity-50 group-hover:opacity-100 transition-opacity ${entryForCell.clientId ? '' : 'text-slate-500'}`} style={entryForCell.clientId ? { color: dynamicStyles.color } : {}} />
                                  </div>
                                </div>
                                </td>
                            );
