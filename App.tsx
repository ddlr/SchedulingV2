...
import { Client, Staff, StaffRole, GeneratedSchedule, DayOfWeek, Team, ScheduleEntry, SessionType, BaseScheduleConfig, ValidationError, Callout, CalloutFormValues, AlliedHealthNeed, BulkOperationSummary, InsuranceQualification } from './types';
import { DAYS_OF_WEEK, PALETTE_ICON_SVG, TEAM_COLORS, ALL_STAFF_ROLES, ALL_SESSION_TYPES, TIME_SLOTS_H_MM, COMPANY_OPERATING_HOURS_START, COMPANY_OPERATING_HOURS_END } from './constants';
...
const App: React.FC = () => {
  ...
  const [activeTab, setActiveTab] = useState<'clients' | 'staff' | 'schedule' | 'baseSchedules' | 'callouts' | 'settings' | 'adminSettings'>('clients');
  const [selectedDate, setSelectedDate] = useState<Date | null>(null);

  const [isSessionModalOpen, setIsSessionModalOpen] = useState(false);
  const [sessionToEdit, setSessionToEdit] = useState<ScheduleEntry | null>(null);
  const [newSessionSlotDetails, setNewSessionSlotDetails] = useState<{ staffId: string; staffName: string; startTime: string; day: DayOfWeek } | null>(null);
  ...
  useEffect(() => {
    ...
    return () => {
      unsubClients(); unsubStaff(); unsubTeams(); unsubQualifications(); unsubBaseSchedules(); unsubCallouts(); unsubSystemConfig();
    };
  }, []);
  ...
  const handleAddStaff = () => { setError(null); staffService.addStaff({ name: 'New Staff', role: 'BT', teamId: '', qualifications: [], canProvideAlliedHealth: [] }); };
  const handleUpdateStaff = (updatedStaff: Staff) => staffService.updateStaff(updatedStaff);
  const handleRemoveStaff = (staffId: string) => staffService.removeStaff(staffId);
  ...
  const handleMoveScheduleEntry = useCallback((
    draggedEntryId: string, newStaffId: string, newStartTime: string
  ) => {
    setError(null);
    ...
    const newStaff = staff.find(t => t.id === newStaffId);
    if (!newStaff) return;
    ...
    const validationErrors = validateFullSchedule(newUpdatedSchedule, clients, staff, availableInsuranceQualifications, selectedDate, COMPANY_OPERATING_HOURS_START, COMPANY_OPERATING_HOURS_END, callouts);
    if (validationErrors.length > 0) {
      setError(validationErrors);
    } else {
      setError(null);
    }
  }, [clients, staff, selectedDate, callouts, schedule]);
  ...
  const handleOpenAddSessionModal = (staffId: string, staffName: string, startTime: string, day: DayOfWeek) => {
    setError(null); 
    setGaStatusMessage(null); 
    setNewSessionSlotDetails({ staffId, staffName, startTime, day });
  ...
  const handleViewBaseSchedule = (baseScheduleId: string) => {
    ...
    if (baseConfig && baseConfig.schedule) {
      ...
      if (newSelectedDate) {
          const validationErrors = validateFullSchedule(scheduleWithIds, clients, staff, availableInsuranceQualifications, newSelectedDate, COMPANY_OPERATING_HOURS_START, COMPANY_OPERATING_HOURS_END, callouts);
          if (validationErrors.length > 0) {
              setError(validationErrors);
          } else {
              setError(null);
  ...
  const handleAddCallout = (e: React.FormEvent) => {
    ...
    const sourceList = entityType === 'client' ? clients : staff;
  ...
  const handleGenerateSchedule = useCallback(async () => {
    ...
    if (clients.length === 0 || staff.length === 0) { setError([{ ruleId: "MISSING_DATA", message: "Add clients and staff." }]); return; }
    ...
    try {
      ...
      const result = await runCsoAlgorithm(clients, staff, availableInsuranceQualifications, selectedDate, callouts, schedule || []);
  ...
  }, [clients, staff, selectedDate, callouts]);
  ...
  const handleStaffFilterChange = (ids: string[]) => setSelectedStaffIds(ids);
  const handleClientFilterChange = (ids: string[]) => setSelectedClientIds(ids);
  const handleClearFilters = () => { setSelectedTeamIds([]); setSelectedStaffIds([]); setSelectedClientIds([]); };
  ...
  const displayedStaff = useMemo(() => {
    ...
    if (selectedStaffIds.length > 0) {
        result = result.filter(t => selectedStaffIds.includes(t.id));
    }
    return result.sort((a,b) => a.name.localeCompare(b.name));
  }, [staff, selectedTeamIds, selectedStaffIds]);

  const displayedSchedule = useMemo(() => {
    if (!schedule) return null;

    const visibleStaffIds = new Set(displayedStaff.map(t => t.id));

    let filteredEntries = schedule.filter(entry => {
        if (!visibleStaffIds.has(entry.staffId)) {
            return false;
        }
  ...
  }, [schedule, selectedClientIds, displayedStaff]);
  ...
  const handleBulkUpdateStaff = async (file: File, action: 'ADD_UPDATE' | 'REMOVE'): Promise<BulkOperationSummary> => {
    setLoadingState({active: true, message: "Processing staff CSV..."}); setBulkOperationSummary(null); setError(null);
    ...
    try {
        ...
        const headers = rows[0].split(',').map(h => h.trim().toLowerCase());
        summary.processedRows = rows.length - 1;
        const staffToProcess: Partial<Staff>[] = [];
        const staffNamesToRemove: string[] = [];
        ...
        for (let i = 1; i < rows.length; i++) {
            ...
            if (!rowData.name) { summary.errors.push({ rowNumber: i + 1, message: "Missing 'name' column.", rowData: rows[i] }); summary.errorCount++; continue; }
            if (rowData.action.toUpperCase() === 'ADD_UPDATE' && action === 'ADD_UPDATE') {
                const staffTeam = availableTeams.find(t => t.name.toLowerCase() === rowData.teamname?.toLowerCase());
                ...
                const partialStaff: Partial<Staff> = { name: rowData.name };
                if (rowData.role) partialStaff.role = rowData.role as StaffRole;
                if (staffTeam !== undefined || rowData.teamname !== undefined) partialStaff.teamId = staffTeam?.id;
                if (qualifications !== undefined) partialStaff.qualifications = qualifications;
                if (canProvideAH !== undefined) partialStaff.canProvideAlliedHealth = canProvideAH;
                staffToProcess.push(partialStaff);
            } else if (rowData.action.toUpperCase() === 'REMOVE' && action === 'REMOVE') staffNamesToRemove.push(rowData.name);
        }
        if (action === 'ADD_UPDATE' && staffToProcess.length > 0) { const result = await staffService.addOrUpdateBulkStaff(staffToProcess); summary.addedCount = result.addedCount; summary.updatedCount = result.updatedCount; }
        else if (action === 'REMOVE' && staffNamesToRemove.length > 0) { const result = await staffService.removeStaffByNames(staffNamesToRemove); summary.removedCount = result.removedCount; }
        const currentIQs = settingsService.getInsuranceQualifications();
        const currentIQNames = currentIQs.map(q => q.id);
  ...
  // Defined inside component to access clients and staff state
  const getCurrentCalloutEntityList = useCallback(() => {
    if (calloutForm.entityType === 'client') {
        ...
        return staff.map(t => ({ id: t.id, name: t.name })).sort((a, b) => a.name.localeCompare(b.name));
    }
  }, [calloutForm.entityType, clients, staff]);
        ...
        <div className="container mx-auto px-4 sm:px-6 py-4 flex flex-col sm:flex-row justify-between items-center space-y-3 sm:space-y-0">
          ...
          <div className="flex flex-col sm:flex-row items-center space-y-3 sm:space-y-0 sm:space-x-2 md:space-x-3">
            ...
            <button
              ...
              disabled={loadingState.active || clients.length === 0 || staff.length === 0 || !selectedDate}
              ...
              aria-live="polite"
              title={!selectedDate ? "Please select a date first" : (clients.length === 0 || staff.length === 0 ? "Add clients and staff first" : `Generate schedule using CSO`)}
            >
            ...
            <button
              ...
              disabled={loadingState.active || !schedule || !selectedDate || clients.length === 0 || staff.length === 0}
              ...
              aria-live="polite"
              title={!schedule || !selectedDate ? "Load or generate a schedule first" : (clients.length === 0 || staff.length === 0 ? "Client/Staff data missing" : "Optimize current schedule with CSO Algorithm")}
            >
      ...
      <main className="container mx-auto px-4 sm:px-6 py-8 flex-grow">
        <div className="bg-white p-1 sm:p-2 rounded-lg shadow-xl mb-8">
          <div className="flex border-b border-slate-200 overflow-x-auto">
            <TabButton tabName="clients" label="Clients" icon={<UserGroupIcon className="w-5 h-5" />} /> <TabButton tabName="staff" label="Staff" icon={<UserGroupIcon className="w-5 h-5" />} /> <TabButton tabName="schedule" label="View Schedule" icon={<ClockIcon className="w-5 h-5" />} /> <TabButton tabName="baseSchedules" label="Base Schedules" icon={<ClipboardDocumentListIcon className="w-5 h-5" />} /> <TabButton tabName="callouts" label="Callouts" icon={<ClipboardDocumentListIcon className="w-5 h-5" />} /> <TabButton tabName="settings" label="Settings" icon={<PaletteIconComponent />} /> <TabButton tabName="adminSettings" label="Admin" icon={<Cog8ToothIcon className="w-5 h-5" />} />
          </div>
